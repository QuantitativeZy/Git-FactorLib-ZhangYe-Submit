...
import backtest
import cal_metric
import dataGet_superNew as dataGet
...
def initialize(context):
    ...
    # config中设置窗口长度
    window1 = context.config['window1']
    window2 = context.config['window2']

    #  ——————————————————————————————以下为因子逻辑——————————————————————————————
    # 读取股票日线格数据存储在全局对象中
    g.stock = dataGet.get_tushare_daily(data_dir=context.data_dir, security=context.security)

    g.stock['dmi'] = DMI(g.stock, window1, window2)
...
def handle_data(context):
    ...
    price = dataGet.get_price(context.data_dir, context.current_dt, context.security, 'open')
    # 子账户3：全仓持有标的
    backtest.order_value(context, context.security, "stock", price, context.portfolios[3].available_cash, 3)

    # 从第二个交易日开始
    if context.current_dt in g.stock.index[2:]:
        # 当前日期在g.stock.index中的索引
        current_index = g.stock.index.to_list().index(context.current_dt)

        # 如果 昨日dmi > 0 且 昨日dmi > 前日dmi
        if g.stock['dmi'].iloc[current_index - 1] > 0 and g.stock['dmi'].diff(1).iloc[current_index - 1] > 0:
            # 如果股票不在子账户1的持仓中，全仓买入
        ...
        # 如果 dmi < 0 且 今日dmi < 昨日dmi
        elif g.stock['dmi'].iloc[current_index - 1] < 0 and g.stock['dmi'].diff(1).iloc[current_index - 1] < 0:
            # 如果股票在子账户1的持仓中，清仓
...
def multi_run(CONF={}):
    ...
    # 生成配置组合
    configs = local_config.generate_config_combinations(CONF_merge)
