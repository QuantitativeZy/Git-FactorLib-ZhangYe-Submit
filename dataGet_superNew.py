...
def get_today_data(data_dir, date, security):
    ...
    """
    try:
        today_data = get_tushare_daily(data_dir, security).loc[date, :]
    except KeyError:
...
def get_portfolio(func):
    def wrapper(data_dir, current_dt, security, price_type='close'):
        if isinstance(security, list):
            prices = pd.DataFrame(columns=['security', 'price'])
            prices.set_index('security', inplace=True)
            for stock in security:
                price = func(data_dir, current_dt, stock, price_type)
                prices.loc[stock] = price  # Todo: 换成DataFrame
            return prices
        else:
            return func(data_dir, current_dt, security, price_type)

    return wrapper


@get_portfolio
def get_price(data_dir, current_dt, security, price_type="close"):
    today_data = get_today_data(data_dir, current_dt, security)
    # 如果当天数据为空，代表停牌，价格等于0
    if len(today_data) == 0:
        price = 0
    else:
        # 使用price_type作为股票当前价格
        price = today_data[price_type]
    return price
...
if __name__ == '__main__':
    # df = attribute_datarange_history(data_dir="data/", security='000001', start_date="2015-01-01",
    #                                  end_date="2021-12-30",
    #                                  frequency='monthly', save=False)
    df = get_capacity(data_dir="data", current_dt=datetime.date(2023, 10, 9), security="603176.SH")
    print("done")
